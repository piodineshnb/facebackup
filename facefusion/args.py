...
def apply_args(args : Args, apply_state_item : ApplyStateItem) -> None:
	...
	# Handle source paths - support both --source-paths and --source
	source_paths = args.get('source_paths')
	if args.get('source'):
		# Parse comma-separated source paths
		source_paths = [path.strip() for path in args.get('source').split(',') if path.strip()]
	apply_state_item('source_paths', source_paths)
	...
	# Handle reference face path - support comma-separated paths
	reference_face_path = args.get('reference_face_path')
	if reference_face_path and ',' in reference_face_path:
		# Parse comma-separated reference face paths
		reference_face_paths = [path.strip() for path in reference_face_path.split(',') if path.strip()]
		apply_state_item('reference_face_paths', reference_face_paths)
		apply_state_item('reference_face_path', reference_face_paths[0] if reference_face_paths else None)  # Keep original for backward compatibility
	else:
		apply_state_item('reference_face_path', reference_face_path)
	# face masker
