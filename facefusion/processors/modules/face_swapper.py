...
def swap_face(source_face : Face, target_face : Face, temp_vision_frame : VisionFrame, source_path : str = None) -> VisionFrame:
	...
	for pixel_boost_vision_frame in pixel_boost_vision_frames:
		...
		pixel_boost_vision_frame = forward_swap_face(source_face, pixel_boost_vision_frame, source_path)
...
def forward_swap_face(source_face : Face, crop_vision_frame : VisionFrame, source_path : str = None) -> VisionFrame:
	...
	for face_swapper_input in face_swapper.get_inputs():
		if face_swapper_input.name == 'source':
			if model_type in [ 'blendswap', 'uniface' ]:
				face_swapper_inputs[face_swapper_input.name] = prepare_source_frame(source_face, source_path)
			else:
...
def prepare_source_frame(source_face : Face, source_path : str = None) -> VisionFrame:
	model_type = get_model_options().get('type')
...
def process_frame(inputs : FaceSwapperInputs) -> VisionFrame:
	...
	source_faces = inputs.get('source_faces', [inputs.get('source_face')])  # Support both single and multiple source faces
	target_vision_frame = inputs.get('target_vision_frame')
	source_paths = inputs.get('source_paths', state_manager.get_item('source_paths'))  # Get from inputs first, fallback to state manager
		...
		if many_faces:
			for target_face in many_faces:
				for i, source_face in enumerate(source_faces):
					source_path = source_paths[i] if i < len(source_paths) else None
					target_vision_frame = swap_face(source_face, target_face, target_vision_frame, source_path)
	...
	if state_manager.get_item('face_selector_mode') == 'reference':
		# Create a mapping of reference faces to their corresponding similar faces
		reference_to_similar_mapping = {}
		if reference_faces:
			for reference_set in reference_faces:
				for reference_face in reference_faces[reference_set]:
					for face in many_faces:
						if compare_faces(face, reference_face, state_manager.get_item('reference_face_distance')):
							reference_to_similar_mapping[reference_set] = face
							break  # Take the first matching face for each reference
		
		# Apply source faces to their corresponding reference faces
		if reference_to_similar_mapping and len(source_faces) > 0:
			# Get reference sets in order (origin_0, origin_1, etc.)
			reference_sets = sorted(reference_to_similar_mapping.keys())
			
			# Apply each source face to its corresponding reference face
			for reference_set in reference_sets:
				# Find the index of this reference set in the original order
				reference_index = int(reference_set.split('_')[1]) if '_' in reference_set else 0
				if reference_index < len(source_faces):
					similar_face = reference_to_similar_mapping[reference_set]
					source_face = source_faces[reference_index]
					source_path = source_paths[reference_index] if reference_index < len(source_paths) else None
					target_vision_frame = swap_face(source_face, similar_face, target_vision_frame, source_path)
...
def process_frames(source_paths : List[str], queue_payloads : List[QueuePayload], update_progress : UpdateProgress) -> None:
	...
	for queue_payload in process_manager.manage(queue_payloads):
		...
		output_vision_frame = process_frame(
		{
			...
			'source_paths': source_paths  # Pass source paths to process_frame
		})
